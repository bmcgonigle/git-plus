...
getCommands = ->
  ...
  git.getRepo()
    .then (repo) ->
      ...
      commands.push ['git-plus:add-all-commit-and-push-upstream', 'Add All, Commit And Push Upstream', -> git.add(repo).then -> GitCommit(repo) -> GitPush(repo, setUpstream: true)]
