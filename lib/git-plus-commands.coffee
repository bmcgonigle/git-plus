...
getCommands = ->
  ...
  git.getRepo()
    .then (repo) ->
      ...
      commands.push ['git-plus:add-all-commit-and-push-upstream', 'Add All, Commit And Push Upstream', -> git.add(repo).then -> GitCommit(repo).then -> GitPush(repo, setUpstream: true)]
